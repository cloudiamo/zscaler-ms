...
# Install zscaler-ms {{{
COPY --chown=1000:1000 . /opt/fmc_repository/zscaler-ms
RUN install_repo_deps.sh /opt/fmc_repository/zscaler-ms/

# Cleanup repository {{{
RUN rm -rf /opt/fmc_repository/zscaler-ms/{.git,docker,Dockerfile}
# }}}
# Build tarball {{{
